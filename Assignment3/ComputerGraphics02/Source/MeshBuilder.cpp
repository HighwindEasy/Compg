...
Mesh* MeshBuilder::GenerateCube(const std::string &meshName, Color color)
{
	...
	v.pos.Set(-0.5f, -0.5f, -0.5f);	v.color = color; v.normal.Set();	vertex_buffer_data.push_back(v);
	v.pos.Set(-0.5f, -0.5f, 0.5f);	v.color = color; v.normal.Set();	vertex_buffer_data.push_back(v);
	v.pos.Set(-0.5f, 0.5f, 0.5f);	v.color = color; v.normal.Set();	vertex_buffer_data.push_back(v);
	v.pos.Set(0.5f, 0.5f, -0.5f);	v.color = color; v.normal.Set();	vertex_buffer_data.push_back(v);
	v.pos.Set(-0.5f, -0.5f, -0.5f);	v.color = color; v.normal.Set();	vertex_buffer_data.push_back(v);
...
Mesh* MeshBuilder::GenerateHalfCone(const std::string &meshName, Color color, unsigned numSlices)
{
	std::vector<Vertex> vertex_buffer_data;
	std::vector<GLuint> index_buffer_data;
	Vertex v;
...
Mesh* MeshBuilder::GenerateText(const std::string &meshName, unsigned numRow, unsigned numCol)
{
	...
	for (unsigned i = 0; i < numRow; ++i)
	{
		for (unsigned j = 0; j < numCol; ++j)
		{
			...
			v.pos.Set(-0.5f, -0.5f, 0);	v.texCoord.Set(u1, v1);						v.normal.Set(0, 0, 1);
			vertex_buffer_data.push_back(v);
			v.pos.Set(0.5f, -0.5f, 0);	v.texCoord.Set(u1 + width, v1);				v.normal.Set(0, 0, 1);
			vertex_buffer_data.push_back(v);
			v.pos.Set(0.5f, 0.5f, 0);	v.texCoord.Set(u1 + width, v1 + height);	v.normal.Set(0, 0, 1);
			vertex_buffer_data.push_back(v);
			v.pos.Set(-0.5f, 0.5f, 0);	v.texCoord.Set(u1, v1 + height);			v.normal.Set(0, 0, 1);
			vertex_buffer_data.push_back(v);
